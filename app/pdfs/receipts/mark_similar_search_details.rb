...
def mark_similar_search_detail(receipt)
    ...
    data =
      ...
      if @receipt.number_classes.positive?
        data += [["746-08", "Taxe au delà de la 3ème classe/ ou par déposant dans 3 classes", "%02d" % @receipt.number_classes, money_format(@receipt.class_tax), money_format(@receipt.class_tax*@receipt.number_classes)]]
      end
